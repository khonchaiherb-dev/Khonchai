...
{id:1,slug:'rang-jued-tea',sku:'KCH-TEA-001',name:'ชารางจืดคัดพิเศษ',desc:'ใบคัดคุณภาพ กลิ่นหอม ดื่มง่าย',price:189,old:259,rating:4.9,sold:1248,stock:86,cat:'ชาสมุนไพร',emoji:'🌿',tag:'ขายดี',featured:true},
...
function normalizeProduct(p){const fallback=FALLBACK_PRODUCTS.find(x=>x.id===Number(p.id))||{};return {id:Number(p.id),slug:p.slug||fallback.slug||'',sku:p.sku||fallback.sku||'',name:p.name||fallback.name||'',desc:p.description??p.desc??fallback.desc??'',price:Number(p.price??fallback.price??0),old:Number(p.compare_at_price??p.old??fallback.old??p.price??0),rating:Number(p.rating??fallback.rating??5),sold:Number(p.sold_count??p.sold??fallback.sold??0),stock:Number(p.stock??fallback.stock??0),cat:p.category??p.cat??fallback.cat??'สินค้า',emoji:p.emoji||fallback.emoji||'🌿',tag:p.tag||fallback.tag||(p.featured?'แนะนำ':''),featured:Boolean(Number(p.featured??fallback.featured??0)),image:p.image_url||p.image||fallback.image||''}}
async function hydrateCatalog(){try{const data=await api('/api/products');if(Array.isArray(data.products)&&data.products.length){PRODUCTS=data.products.map(normalizeProduct);cart=cart.map(x=>{const fresh=PRODUCTS.find(p=>p.id===Number(x.id));return fresh?{...fresh,qty:x.qty}:x});save()}}catch{}try{const data=await api('/api/coupons');if(Array.isArray(data.coupons)&&data.coupons.length){COUPONS=data.coupons.map(c=>({...c,label:c.code==='WELCOME50'?'ลูกค้าใหม่ ลด ฿50':c.type==='percent'?`ลด ${c.value}%${c.max_discount?` สูงสุด ${M(c.max_discount)}`:''}`:`ลด ${M(c.value)}`}))}}catch{}if(current==='home')home()}
function visual(p,cls=''){const content=p.image?`<img src="${esc(p.image)}" alt="${esc(p.name)}" loading="lazy">`:`<span class="emoji">${p.emoji||'🌿'}</span>`;return `<div class="visual ${cls}">${p.tag?`<span class="tag">${esc(p.tag)}</span>`:''}${content}<span class="decor d1">☘</span><span class="decor d2">❧</span></div>`}
