...
export function legacyAuthorized(request,env){if(String(env.LEGACY_ADMIN_ENABLED??'true').toLowerCase()==='false')return false;const expected=String(env.ADMIN_TOKEN||''),got=String(request.headers.get('Authorization')||'').replace(/^Bearer\s+/i,'');if(!expected||got.length!==expected.length)return false;let diff=0;for(let i=0;i<expected.length;i++)diff|=expected.charCodeAt(i)^got.charCodeAt(i);return diff===0}
...
export async function getStaffSession(request,env,{touch=true}={}){
  ...
  let sessionRef=null;try{const meta=await env.DB.prepare("SELECT session_ref FROM staff_session_meta WHERE token_hash=?").bind(hash).first();sessionRef=meta?.session_ref||null}catch{}
  return {id:Number(row.staff_user_id),username:row.username,displayName:row.display_name,role:row.role,tokenHash:hash,sessionRef,expiresAt:row.expires_at,permissions:permissionsFor(row.role)};
}
...
export async function issueStaffCsrf(request,env,staff){
  if(!staff?.tokenHash||!(await tableExists(env,'staff_session_meta')))return {csrfToken:null,sessionRef:staff?.sessionRef||null,available:false};
  const existing=await env.DB.prepare("SELECT session_ref FROM staff_session_meta WHERE token_hash=?").bind(staff.tokenHash).first();
  const sessionRef=existing?.session_ref||randomHex(12),csrfToken=randomToken(24),csrfHash=await sha256Hex(`${staff.tokenHash}|${csrfToken}`),uaHash=await sha256Hex(String(request.headers.get('User-Agent')||'').slice(0,500));
  await env.DB.prepare(`INSERT INTO staff_session_meta(token_hash,session_ref,csrf_token_hash,device_label,user_agent_hash)
    VALUES(?,?,?,?,?) ON CONFLICT(token_hash) DO UPDATE SET csrf_token_hash=excluded.csrf_token_hash,device_label=excluded.device_label,user_agent_hash=excluded.user_agent_hash,updated_at=datetime('now')`)
    .bind(staff.tokenHash,sessionRef,csrfHash,deviceLabel(request),uaHash).run();
  return {csrfToken,sessionRef,available:true};
}
export async function verifyStaffCsrf(request,env,staff){
  if(!sameOrigin(request)||!staff?.tokenHash)return false;
  if(!(await tableExists(env,'staff_session_meta')))return null;
  const token=String(request.headers.get('X-KCH-CSRF')||'');if(token.length<20||token.length>200)return false;
  const row=await env.DB.prepare("SELECT csrf_token_hash FROM staff_session_meta WHERE token_hash=?").bind(staff.tokenHash).first();if(!row?.csrf_token_hash)return false;
  const got=await sha256Hex(`${staff.tokenHash}|${token}`);return constantTimeEqual(got,row.csrf_token_hash);
}
