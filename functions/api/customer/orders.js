...
export async function onRequestGet({request,env}){
  ...
  for(const o of r.results||[]){
    const [items,events]=await Promise.all([
      env.DB.prepare(`SELECT oi.id order_item_id,oi.product_id,oi.variant_id,oi.product_name,oi.unit_price,oi.qty,oi.line_total,p.slug,
    ...
    orders.push({...o,items:items.results||[],shipmentEvents:events.results||[]});
  }
  return json({orders});
}
