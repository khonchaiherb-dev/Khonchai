...
export async function onRequestGet({env}){
  ...
  const tables=['staff_users','staff_sessions','staff_session_meta','security_events','approval_requests','approval_decisions','approval_executions','backup_snapshots'];
  const tableState={};for(const t of tables)tableState[t]=await tableExists(env,t);
  let staff={owners:0,admins:0,active:0},critical24h=0,pendingApprovals=0;
  ...
  return json({version:'1.5.0',ready:requiredPassed===required.length,score,requiredPassed,requiredTotal:required.length,staff,critical24h,pendingApprovals,checks});
}
