...
export async function onRequestGet({env}){
  ...
  const tables=['staff_users','staff_sessions','staff_session_meta','security_events','approval_requests','approval_decisions','approval_executions','backup_snapshots','order_request_registry','maintenance_runs','system_integrity_snapshots'];
  ...
  const resilienceSchema=tableState.order_request_registry&&tableState.maintenance_runs&&tableState.system_integrity_snapshots,orderGuard=bool(env.ORDER_IDEMPOTENCY_REQUIRED)||bool(env.PRODUCTION_MODE)||String(env.ENVIRONMENT||'').toLowerCase()==='production';
  const checks=[
  ...
  return json({version:'1.6.0',ready:requiredPassed===required.length,score,requiredPassed,requiredTotal:required.length,staff,critical24h,pendingApprovals,latestIntegrity,checks});
}
