...
export async function onRequestGet({request,env}){
  ...
  const p=id
    ? await env.DB.prepare(`SELECT id,slug,sku,name,description,category,price,compare_at_price,rating,sold_count,stock,COALESCE(reserved_stock,0) reserved_stock,featured,barcode,weight_grams,seo_title,seo_description,sale_verified FROM products WHERE id=? AND active=1 AND sale_verified=1 AND price>0`).bind(id).first()
    : await env.DB.prepare(`SELECT id,slug,sku,name,description,category,price,compare_at_price,rating,sold_count,stock,COALESCE(reserved_stock,0) reserved_stock,featured,barcode,weight_grams,seo_title,seo_description,sale_verified FROM products WHERE slug=? AND active=1 AND sale_verified=1 AND price>0`).bind(slug).first();
  ...
  if(!(media.results||[]).length)return json({error:'not_found'},404,{'Cache-Control':'no-store'});
  ...
  if(out.available_stock<=0&&!out.variants.some(v=>v.available_stock>0))return json({error:'not_found'},404,{'Cache-Control':'no-store'});
