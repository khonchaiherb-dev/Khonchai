...
export async function onRequestGet({env}){
  ...
  const r=await env.DB.prepare("SELECT code,type,value,min_spend,max_discount,new_customer_only FROM coupons WHERE active=1 AND (starts_at IS NULL OR starts_at<=datetime('now')) AND (ends_at IS NULL OR ends_at>=datetime('now'))").all();
