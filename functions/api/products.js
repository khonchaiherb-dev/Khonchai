export async function onRequestGet({env,request}){
  ...
  let sql=`SELECT p.id,p.slug,p.sku,p.name,p.description,p.category,p.price,p.compare_at_price,p.rating,p.sold_count,
    MAX(0,p.stock-COALESCE(p.reserved_stock,0)) stock,p.stock on_hand_stock,COALESCE(p.reserved_stock,0) reserved_stock,p.featured,
  ...
  return Response.json({products:r.results||[]},{headers:{'Cache-Control':'public, max-age=30'}});
}
