import {json,sameOrigin,safeUsername,derivePassword,randomToken,sha256Hex,securityEvent,issueStaffCsrf} from '../../_lib/staff-auth.js';
const maxAge=28800;
export async function onRequestPost({request,env}){
  ...
  const sec=await issueStaffCsrf(request,env,{id:Number(u.id),tokenHash,sessionRef:null});
  await securityEvent(env,{staffUserId:u.id,eventType:'staff_login_success',route:'/api/staff/login',method:'POST',statusCode:200,detail:{sessionSecurity:sec.available?'v1.5':'v1.4-compatible'}});
  return json({ok:true,staff:{id:u.id,username:u.username,displayName:u.display_name,role:u.role},csrfToken:sec.csrfToken,sessionRef:sec.sessionRef,sessionSecurity:sec.available},200,{'Set-Cookie':`kch_staff=${encodeURIComponent(token)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${ttl}`});
}
