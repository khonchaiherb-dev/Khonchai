-- K-EXAM centralized item timing v6
-- Adds active per-question timing using first response per anonymous user.
-- Provides average/median time and a slow-item list without collecting identity data.
-- Safe to apply after 20260918_kexam_quality_review_v5.sql.
...
item_time_rows as (
  select b.id event_id,
         b.position_key,
         b.anonymous_hash,
         split_part(t.value,'|',1) question_id,
         least(split_part(t.value,'|',2)::int,3600)::int time_seconds
  from submit_base b
  cross join lateral jsonb_array_elements_text(
    case when jsonb_typeof(b.metadata->'item_times')='array'
      then b.metadata->'item_times' else '[]'::jsonb end
  ) t(value)
  where t.value ~ '^(RA|TA)-S[0-9]{2}-Q[0-9]{3}\|[0-9]{1,5}$'
),
item_first as (
...
item_agg as (
  select f.position_key,f.question_id,
         ...
         count(t.time_seconds)::int time_samples,
         round(avg(t.time_seconds)::numeric,1) avg_time_seconds,
         round((percentile_cont(0.5) within group(order by t.time_seconds))::numeric,1) median_time_seconds
  from item_first f
  join item_attempts a
    on a.position_key is not distinct from f.position_key
   and a.question_id=f.question_id
  left join item_time_rows t
    on t.event_id=f.event_id
   and t.question_id=f.question_id
...
item_stats as (
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'time_basis','first-response-per-user-active-time',
...
slow_items as (
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'discrimination',discrimination,
    'time_basis','first-response-per-user-active-time'
  ) order by median_time_seconds desc,avg_time_seconds desc,question_id asc),'[]'::jsonb) value
  from (
    select *
    from item_agg
    where time_samples>=5
      and median_time_seconds is not null
    order by median_time_seconds desc,avg_time_seconds desc,question_id asc
    limit 40
  ) x
),
report_counts as (
...
select jsonb_build_object(
  ...
  'slow_items',(select value from slow_items),
