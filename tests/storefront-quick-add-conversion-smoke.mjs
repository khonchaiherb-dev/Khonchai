...
for(const token of ['data-add="${Number(actual.id)}"','aria-label="เพิ่ม ${esc(actual.name)} ลงตะกร้า"','shopping_cart'])if(!master.includes(token))throw new Error(`master product card quick-add contract missing ${token}`);
