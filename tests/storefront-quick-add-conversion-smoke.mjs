...
for(const token of ['async function v17Purchase','if(!variant){',"product(p);toast('สินค้านี้มีหลายตัวเลือก กรุณาเลือกก่อนสั่งซื้อ');return false",'V17_SELECTED_VARIANT.set(Number(p.id),variant.id)','const ok=v17AddVariant(p,variant,q);if(ok&&buyNow)checkout();return ok;'])if(!variants.includes(token))throw new Error(`variant-safe quick-add behavior missing ${token}`);
