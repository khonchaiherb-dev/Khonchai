...
const structuralCss=(middleware.match(/tshop-v134-structural-storefront\.css\?v=([0-9.]+)/)||[])[1];
const structuralJs=(middleware.match(/kch-v134-structural-storefront\.js\?v=([0-9.]+)/)||[])[1];
assert.equal(structuralCss,'1.34.6','middleware must publish urgent structural CSS v1.34.6');
assert.equal(structuralJs,'1.34.6','middleware must publish urgent structural runtime v1.34.6');
assert.ok(sw.includes(`/tshop-v134-structural-storefront.css?v=${structuralCss}`),'service worker must cache current structural CSS');
assert.ok(sw.includes(`/kch-v134-structural-storefront.js?v=${structuralJs}`),'service worker must cache current structural runtime');
