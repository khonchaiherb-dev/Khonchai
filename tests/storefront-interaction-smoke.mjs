...
const calls=[];let clickHandler=null;
const document={readyState:'complete',documentElement:{dataset:{}},addEventListener(type,fn){if(type==='click')clickHandler=fn},querySelector(){return null},querySelectorAll(){return []}};
...
vm.createContext(context);vm.runInContext(source,context,{filename:'tshop-v161-hotfix.js'});assert.equal(typeof clickHandler,'function');
function element(kind,dataset={}){const selector={add:'[data-add]',buy:'[data-buy]',drawerAdd:'[data-v05-drawer-add]',drawerBuy:'[data-v05-drawer-buy]',place:'[data-place]',cart:'[data-go="cart"]',checkout:'[data-go="checkout"]',product:'[data-product]'}[kind];return {dataset,disabled:false,getAttribute(){return null},matches(q){return q===selector},closest(q){if(q.includes(selector))return this;if(kind==='product'&&q==='button,a,input,select,textarea,label')return null;return null}}}
function fire(el){let prevented=0,stopped=0;clickHandler({target:el,preventDefault(){prevented++},stopImmediatePropagation(){stopped++}});assert.equal(prevented,1);assert.equal(stopped,1)}
...
assert.match(source,/1\.7\.0/);assert.match(source,/data-v05-drawer-add/);assert.match(source,/v17Purchase/);assert.match(source,/stopImmediatePropagation/);
