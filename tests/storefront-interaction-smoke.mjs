...
const version=source.match(/__KCH_INTERACTION_HOTFIX__='(\d+)\.(\d+)\.(\d+)'/);assert.ok(version);assert.deepEqual(version.slice(1).map(Number),[1,7,2]);
...
console.log('storefront interaction + mobile pointer transparency + direct touch/redraw-resilient product tap smoke: OK');
