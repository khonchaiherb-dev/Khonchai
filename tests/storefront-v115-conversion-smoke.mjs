...
assert.match(css,/\.v115-desktop-nav/);assert.match(css,/\.v115-pdp-layout\{display:grid;grid-template-columns/);assert.match(css,/\.v115-checkout-grid\{display:grid;grid-template-columns/);assert.match(css,/\.v115-checkout-aside\{position:sticky/);assert.match(css,/\.tshop-bottom\{display:none!important\}/);assert.match(css,/@media\(max-width:699px\)/);assert.match(css,/display:contents/);assert.match(css,/safe-area-inset-bottom/);assert.match(css,/focus-visible/);assert.match(css,/\.v115-pdp-brandnote/);assert.match(css,/\.v115-pdp-trust/);assert.match(css,/shop-profile\[data-v115-brand/);
assert.match(source,/v115DesktopNav/);assert.match(source,/v115PolishStorefront/);assert.match(source,/v115EnhancePdpBrand/);assert.match(source,/v115PolishCheckoutSource/);assert.match(source,/v115EnhancePdp/);assert.match(source,/v115EnhanceCheckout/);assert.match(source,/v115-checkout-assurance/);assert.match(source,/MutationObserver/);assert.match(source,/v115BindBase/);assert.match(source,/pdp-bottom/);assert.match(source,/sticky-pay/);assert.match(source,/__KCH_CONVERSION_LAYOUT__/);assert.match(source,/เรื่องราวจากคุณชายสมุนไพร/);assert.match(source,/ค้นหาสินค้า คุณชายสมุนไพร/);assert.match(source,/จัดจำหน่ายโดย คุณชายสมุนไพร/);assert.match(source,/ราคาและสต็อก/);assert.match(source,/เก็บเงินปลายทาง/);assert.match(source,/ออกใบเสร็จได้/);assert.doesNotMatch(source,/ระบบจะใช้แหล่งที่มาเพื่อวัดผลยอดขาย/);
assert.ok(pkg.scripts['qa:v115']);assert.match(pkg.scripts.check,/storefront-v115-conversion-smoke\.mjs/);
console.log('storefront v1.15 desktop + mobile conversion + Thai brand PDP trust smoke: OK');
