...
const build=(html.match(/name="kch-build" content="([^"]+)"/)||[])[1],parts=String(build||'').split('.').map(Number);assert.ok(parts[0]>1||(parts[0]===1&&parts[1]>=13),'build must be v1.13+');assert.equal(pkg.version,build);
assert.ok(html.includes(`tshop-v113.css?v=${build}`));assert.ok(html.includes(`tshop-v113.js?v=${build}`));assert.ok(html.indexOf(`tshop-v113.js?v=${build}`)>html.indexOf(`tshop-v112.js?v=${build}`));assert.ok(html.indexOf(`tshop-v113.js?v=${build}`)<html.indexOf(`tshop-v161-hotfix.js?v=${build}`));assert.ok(sw.includes(`khonchaiherb-v${build}`));assert.ok(sw.includes(`/tshop-v113.js?v=${build}`));assert.ok(sw.includes(`/tshop-v113.css?v=${build}`));assert.ok(health.includes(`version:'${build}'`));assert.match(health,/memberRewardWallet:true/);
