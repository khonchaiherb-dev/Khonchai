...
const build=(html.match(/name="kch-build" content="([^"]+)"/)||[])[1];assert.equal(build,'1.12.0');assert.equal(pkg.version,build);
assert.ok(html.includes(`tshop-v112.css?v=${build}`));assert.ok(html.includes(`tshop-v112.js?v=${build}`));assert.ok(html.indexOf(`tshop-v112.js?v=${build}`)>html.indexOf(`tshop-v111.js?v=${build}`));assert.ok(html.indexOf(`tshop-v112.js?v=${build}`)<html.indexOf(`tshop-v161-hotfix.js?v=${build}`));assert.ok(sw.includes(`khonchaiherb-v${build}`));assert.ok(sw.includes(`tshop-v112.js?v=${build}`));assert.match(health,/version:'1\.12\.0'/);assert.match(health,/reviewRewardConfigured:true/);
