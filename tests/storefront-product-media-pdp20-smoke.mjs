...
assert.match(core,/KCH_FLAGSHIP_LAUNCH/);
assert.match(core,/function kchFlagshipAssistant/);
assert.match(core,/function kchFlagshipLaunch/);
assert.match(core,/kch-shop-assistant-form/);
assert.match(core,/บอกสิ่งที่กำลังมองหา/);
assert.match(core,/เตรียมพบสินค้าใหม่จากคุณชายสมุนไพร/);
assert.match(core,/ราคาเร็ว ๆ นี้/);
assert.match(core,/30 ซอง/);
assert.match(core,/100 แคปซูล/);
...
console.log('real product media + PDP 2.0 + flagship discovery + dynamic product SEO smoke: OK');
