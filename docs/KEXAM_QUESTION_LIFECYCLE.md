...
## Risk-based source gate

การเปลี่ยนสถานะจาก `Verified` ไปเป็น `Published` ใช้การตรวจแหล่งอ้างอิงตามความเสี่ยงเพิ่มเติมจาก Structural QA

- ข้อที่มีคะแนนความเสี่ยงด้านแหล่งอ้างอิงตั้งแต่ **70 คะแนนขึ้นไป** ต้องมี `source_state=verified` ก่อนเผยแพร่
- การตรวจแหล่งอ้างอิงต้องมี `source_refs` อย่างน้อย 1 รายการ และผูกกับ SHA-256 ของเนื้อหาปัจจุบัน
- หากแก้เนื้อหาของข้อที่เคย source-verified ต้องย้ายออกจาก Published, refresh content baseline ตามขั้นตอน และตรวจแหล่งอ้างอิงใหม่
- ข้อ Published เดิมที่ยัง pending ไม่ถูกถอนย้อนหลังโดยอัตโนมัติ แต่ยังคงอยู่ใน Source Verification Backlog เพื่อทยอยตรวจตามลำดับความเสี่ยง
- เกณฑ์นี้ออกแบบเพื่อป้องกันข้อที่อ้างมาตรา อัตรา วงเงิน กำหนดเวลา สิทธิ หน้าที่ หรือโทษ ถูกเผยแพร่ใหม่หลังแก้ไขโดยยังไม่มีหลักฐานจากแหล่งทางการ
