(()=>{
  ...
  function academicNextBatchPreviewHtml(preview={}){
    ...
    const ids=rows.slice(0,15).map(r=>`<span class="batch-id">${esc(r.question_id)} · ${esc(String(r.signal||'').toUpperCase())}${Number(r.source_risk_score)>=70?' · SRC '+fmt(r.source_risk_score):''}</span>`).join('');
    const sets=Object.entries(preview.sets||{}).sort((a,b)=>Number(a[0])-Number(b[0])).map(([k,v])=>`ชุด ${k}: ${v}`).join(' · ');
    return `<div class="remediation-next"><div class="remediation-next-head"><h4>Batch ถัดไป · ${fmt(preview.size||rows.length)} ข้อ</h4><span>${esc(String(preview.recommended_level||'').toUpperCase())}</span></div><div class="meta">กระจาย ${esc(sets||'-')} · Source-sensitive ${fmt(preview.source_sensitive||0)} ข้อ</div><div class="batch-id-wrap">${ids}</div></div>`;
  }
