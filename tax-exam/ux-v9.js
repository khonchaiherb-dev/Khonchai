(()=>{
  ...
  function enhance(){
    ...
    document.querySelectorAll('.progressbar').forEach(el=>{
      ...
      const w=el.querySelector('span')?.style.width||'0%';
      el.setAttribute('aria-valuenow',String(parseInt(w)||0));el.setAttribute('aria-valuemin','0');el.setAttribute('aria-valuemax','100');
    });

    const hint=document.querySelector('.submit-hint');
    const submit=document.querySelector('#submitBtn');
    if(hint&&submit?.disabled&&!hint.querySelector('.jump-unanswered')){
      const b=document.createElement('button');b.className='jump-unanswered';b.type='button';b.textContent='ไปยังข้อที่ยังไม่ได้ตอบ';
      b.onclick=()=>{const q=[...document.querySelectorAll('.left-panel .qnum')].find(x=>!x.classList.contains('answered'));q?.click()};
      hint.append(document.createElement('br'),b);
    }

    const question=document.querySelector('.question');
    if(question&&!question.hasAttribute('tabindex'))question.setAttribute('tabindex','-1');
    document.querySelectorAll('.set-card').forEach(card=>{
      const set=card.dataset.set;if(set)card.setAttribute('aria-label',`เปิดข้อสอบชุดที่ ${set}`);
    });
  }
